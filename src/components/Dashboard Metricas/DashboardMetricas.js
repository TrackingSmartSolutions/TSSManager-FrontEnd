...
const CustomDatePickerInput = ({ value, onClick, placeholder }) => (
    <div className="dashboard-date-picker-wrapper">
        <input
            type="text"
            value={value}
            onClick={onClick}
        ...
        />
        <div className="dashboard-date-picker-icons">
            <svg
...
const DashboardMetricas = () => {
    ...
    useEffect(() => {
        ...
        if (!initialDataLoaded) {
            ...
            const rangoSemanaActual = obtenerRangoSemanaActual()
            setDateRange(rangoSemanaActual)
    ...
    useEffect(() => {
        if (initialDataLoaded && (startDate || endDate || selectedUser !== "todos")) {
            fetchDashboardData()
        }
    }, [startDate, endDate, selectedUser, initialDataLoaded])
    ...
    const handleDownloadPDF = async () => {
        try {
            ...
            const formatDateCorrectFromObj = (date) => {
                if (!date) return ''
                const year = date.getFullYear()
                const month = String(date.getMonth() + 1).padStart(2, '0')
                const day = String(date.getDate()).padStart(2, '0')
            ...
            const periodoReporte = `${formatDateCorrectFromObj(startDate)} - ${formatDateCorrectFromObj(endDate)}`
                    ...
                    <div className="reporte-container">
                        <div className="reporte-header">
                            <div className="reporte-header-info">
                                ...
                                <p className="reporte-subtitle">
                                    {currentUser.nombre} {currentUser.apellidos} - {startDate && endDate 
                                        ? `${startDate.toLocaleDateString('es-MX', { day: '2-digit', month: '2-digit', year: 'numeric' })} a ${endDate.toLocaleDateString('es-MX', { day: '2-digit', month: '2-digit', year: 'numeric' })}` 
                                        : formatDate()}
                                </p>
                            </div>
                            <div className="reporte-header-controls">
                                ...
                                <button
                                    className="reporte-btn reporte-btn-secondary"
                                    onClick={() => setDateRange(obtenerRangoSemanaActual())}
